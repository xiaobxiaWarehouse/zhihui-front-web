...
module.exports = {
  ...
  [`POST ${apiPrefix}/user/login`] (req, res) {
    ...
    const user = adminUsers.filter(item => item.account === account && item.mima === mima);
  ...
  [`GET ${apiPrefix}/user/isLogin`] (req, res) {
    ...
    if (response.success) {
      ...
      if (userItem.length > 0) {
        ...
        user.account = userItem[0].account;
  ...
  [`GET ${apiPrefix}/user/getProfile`] (req, res) {
    const cookie = req.headers.cookie || '';
    const cookies = qs.parse(cookie.replace(/\s/g, ''), {
      delimiter: ';',
    });
  ...
  [`POST ${apiPrefix}/user/logout`] (req, res) {
