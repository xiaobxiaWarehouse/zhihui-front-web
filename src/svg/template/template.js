...
export default function (args) {
  const {family, fontPath, unicodes} = args;
  const pathToFonts = fontPath;
  const glyphsFamily = unicodes.reduce((glyphs, glyph) => {
    glyphs[glyph.name] = `\\${glyph.unicode.charCodeAt(0).toString(16).toLowerCase()}`;
    return glyphs;
  }, {});
  const data = {};
  data[family] = glyphsFamily;
  ...
  const str = TEMPLATE.replace(/__FAMILY__|__RELATIVE_FONT_PATH__|goat/gi, (matched) => {
