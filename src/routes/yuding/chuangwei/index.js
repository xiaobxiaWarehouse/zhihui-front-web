...
const Index = (props) => {
  ...
  const filterProps = {
    ...
    onBack () {
      ...
      dispatch({
        type: 'yuding/hideChuangweiModal',
        payload: {
          savaChuangwei,
        },
      });
  ...
  const listProps = {
    ...
    selectBaofang(item) {
      ...
      Object.keys(item).forEach((key) => {
        if (optional) {
          select = optional.every((t) => {
            return t.id === item[key].id && (t.shenqingBaofang === 2 || t.yudingBaofang === 2);
          });
        }
      });
      return select;
    ...
    chuangweiSelect (item) {
      let isModify = chuzhiDz === 3 || chuzhiDz === 4 || (editData && editData.zhuangtai === 2 && chuzhiDz === 2) || (editData && editData.zhuangtai === 3 && chuzhiDz === 2);
      let isSelect = false;
